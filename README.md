...
Этап 4: поясняющие экраны с живыми SVG-рисунками, формулы с легендой, глоссарий, звук, сброс прогресса, мобильная вёрстка. События записываются на «диск» и анализируются офлайн на отдельном экране «Анализ»: именованные отборы по pT, η, зарядам, массе и заполнениям, гистограммы семи переменных с наложением двух отборов, подгонка пика с ошибками, таблица событий с дисплеем события (треки в детекторе), паспорт частицы с моделированием отклика и сравнением с известными частицами, каталог открытий, экспорт CSV. Идеи для этапа 5 в [docs/IDEAS.md](docs/IDEAS.md).
